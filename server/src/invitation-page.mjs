...
export function invitationPage({ invite, code, publicBaseUrl, apkAvailable = false, error } = {}) {
  ...
  if (valid) {
    ...
    content = `${invite.user_id === null ? '<p>서버 운영자가 발급한 첫 기기 초대입니다. 앱에서 표시할 이름을 입력하면 가족용으로 바로 시작할 수 있어요.</p>' : `<p><strong>${escapeHtml(invite.inviter_name)}</strong>님이 가족으로 초대했어요.</p>`}
...
<p><strong>설치가 끝나면 이 페이지로 돌아와 두 번째 버튼을 누르세요.</strong> 앱이 이미 있다면 바로 초대를 열면 됩니다.</p>
<p class="detail">Android 전용입니다. iPhone에서는 설치할 수 없습니다.</p>
<details><summary>설치가 처음이신가요?</summary><ol><li>믿을 수 있는 가족이나 서버 운영자가 보낸 초대인지 확인하고 위에서 APK를 내려받으세요.</li><li>내려받은 파일을 열고 Android의 다운로드·설치 승인 안내를 확인하세요. 필요한 경우 해당 브라우저의 ‘이 출처 허용’을 켜고, 설치 후에는 다시 꺼 주세요. QR만으로 앱이 자동 설치되지는 않습니다.</li><li>이 초대 페이지로 돌아와 ‘앱에서 초대 열기’를 누르세요. 서버 주소와 초대 코드는 자동으로 전달됩니다.</li><li>앱에서 초대한 사람과 서버를 확인하고, 처음 사용하는 기기라면 표시할 이름만 입력한 뒤 직접 참여하세요. 이메일이나 비밀번호는 필요하지 않습니다. 초대가 만료되면 새 초대를 요청하세요.</li></ol><p class="detail">앱이 열리지 않으면 설치 여부를 확인하고 Android의 기본 브라우저에서 이 페이지를 다시 열어 주세요.</p></details>
<p class="notice">연결만으로 위치 공유가 켜지지는 않습니다. 이미 공유 중인 가족의 저장된 위치와 이동 기록은 새로 연결된 가족에게도 보일 수 있습니다.</p>
