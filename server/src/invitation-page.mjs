...
<p><strong>초대 역할: ${roleLabel}</strong> · <span class="detail">${roleLabel === 'guardian' ? '본인과 연결된 가족의 공유 위치를 모두 확인합니다.' : '보호자 위치를 제외한 연결 가족의 공유 위치를 확인합니다.'}</span></p>
...
<p class="notice">연결만으로 위치 공유가 켜지지는 않습니다. 보호자는 연결된 가족의 공유 위치를 모두 볼 수 있고, 피보호자는 보호자 위치와 이동 기록을 볼 수 없습니다.</p>
