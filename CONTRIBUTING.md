...
CI는 고정 커밋의 Actions로 wrapper 무결성 검사, 키 없는 Android debug/unsigned release 빌드, 서버 테스트와 격리된 컨테이너 검증을 수행합니다. APK 업로드·서명·Play 제출은 하지 않습니다. 현재 소스의 target/compile SDK 35와 버전 1.1.0(code 2)을 단순 소스 공개에 맞춰 올리지 않습니다. 향후 Play 제출에는 API 36 이상 전환 등 당시 정책 검증이 별도로 필요합니다.
