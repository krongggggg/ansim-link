...
- **역할별 가족 연결**: 초대할 때 보호자 또는 피보호자 역할을 지정합니다. 보호자는 연결된 피보호자의 동의한 위치를 확인하고, 피보호자는 보호자 위치를 볼 수 없습니다.
- **QR 초대**: 가족이 준비되면 역할이 지정된 1회용 초대를 발급합니다. 초대 링크에는 연결할 서버 정보가 포함됩니다.
- **안심존·안심귀가**: 피보호자가 정한 구역과 귀가 일정을 관리합니다. 동작에는 피보호자 본인의 위치 공유가 필요합니다.
...
- **보호자**는 본인과 직접 연결된 보호자·피보호자가 각자 동의해 공유한 최근 위치와 최근 7일 위치 기록을 모두 조회할 수 있습니다. **피보호자**는 본인과 직접 연결된 피보호자의 공유 위치를 볼 수 있지만, 보호자의 위치·이동 기록·안심귀가·위치가 포함된 SOS는 볼 수 없습니다.
...
- **가족 연결만으로 위치 공유가 시작되지 않습니다.** 각 기기의 위치 공유와 보호자 기기의 백그라운드 알림은 별도 설정입니다. 같은 역할의 가족도 한 사람마다 새 초대를 발급해 여러 명 연결할 수 있습니다.
...
6. 피보호자는 **별도로 공유에 동의하고 켭니다**. 연결된 보호자만 그 위치와 남아 있는 최근 기록을 볼 수 있습니다. 피보호자·다른 보호자에게는 제공되지 않으며, 보호자 역할은 자신의 위치 공유를 켤 수 없습니다.
...
버전 1.3 이하 서버의 기존 프로필은 v3 데이터베이스 이전 뒤 역할이 미선택 상태가 됩니다. 이 상태에서는 다른 사람 위치·이동 기록·위치가 포함된 안전 정보가 숨겨지고 새 위치 전송도 거부됩니다. Android 1.4에서 각 기기가 보호자 또는 피보호자를 한 번 선택해야 하며 앱에서 다시 바꿀 수 없습니다. 보호자를 선택하면 그 프로필의 기존 위치 기록을 삭제하고 진행 중인 안심귀가를 취소합니다. 피보호자를 선택한 뒤에는 기존 공유 동의를 확인하고 이 기기의 위치 수집을 직접 다시 시작하세요. 운영자는 먼저 일관된 저장소 백업을 만든 뒤 **서버를 1.4로 올리고, 이어서 Android 앱을 업데이트**해야 합니다.
...
- Android 앱은 암호화한 기기 세션을 앱 종료·정상 덮어쓰기 업데이트 후에도 유지합니다. 피보호자가 역할 이전을 마친 뒤에는 공유 상태와 권한을 확인하고 위치 수집을 직접 다시 시작해야 할 수 있습니다. 꺼 둔 공유·해제 확인 대기 상태를 자동으로 켜지 않으며, 서버 상태를 확인할 수 없으면 재개하지 않습니다.
- 피보호자의 위치 기록은 서버에서 7일 보관 범위로 관리됩니다. 공유를 끄면 클라이언트는 새 전송을 중단하고 서버에 저장된 해당 프로필의 위치 기록 삭제를 요청합니다. 서버 반영 시 진행 중·예정 시간 경과 상태의 안심귀가도 취소됩니다. **오프라인이면 서버 반영은 보류**되므로 클라이언트의 완료 상태를 확인해야 합니다. 공유 중지는 프로필·가족 연결·모든 종류의 데이터 삭제와 같지 않습니다.
- 가족 연결 해제는 두 사람 사이의 역할별 접근을 끊는 기능이지 상대방의 프로필이나 서버의 위치 기록을 삭제하는 기능이 아닙니다.
