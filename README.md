# 안심연결 서버 · Ansim Link Server

가족의 초대·연결과 동의한 위치 공유를 위한 **Node.js·SQLite 서버**입니다. 이 저장소에서 공개하는 서버 소스는 [MIT License](LICENSE)로 제공됩니다.

**Android 앱은 별도의 비공개 제품이며, 이 공개 저장소에는 Android 소스나 APK가 포함되지 않습니다. Google Play에도 아직 출시되지 않았습니다.** 휴대전화에서 사용하려면 호환되는 클라이언트를 별도로 확보해야 합니다. 서버를 설치하거나 초대 링크를 발급하는 것만으로 앱을 구할 수 있는 것은 아닙니다. 이 안내는 서버 자체의 실행·검증과, 클라이언트를 별도로 확보한 뒤의 기기 연결을 구분합니다.

**긴급 구조 서비스가 아닙니다.** 사고 감지나 알림 도착을 보장하지 않으며 긴급기관에 자동 신고하지 않습니다. 위급하면 앱 알림을 기다리지 말고 현지 긴급전화(한국 112·119)를 이용하세요. 서버 운영·보안·백업과 별도 클라이언트의 사용 가능 여부는 운영자가 확인해야 합니다.

## 기능과 한계

서버는 다음 기능을 제공합니다. 휴대전화 화면·지도·권한 처리·백그라운드 동작은 별도 클라이언트의 역할입니다.
...
별도 안심연결 클라이언트는 Android 전용이며 iOS 앱은 없습니다. NAVER 지도 표시와 위치·알림 권한은 클라이언트에서 처리합니다. Android 화면에는 SOS 촬영·녹음·전송 기능이 없습니다. 공유 중 위치 서비스는 알림을 표시하는 포그라운드 서비스로 동작하며, 오프라인 위치를 디스크에 쌓아 나중에 모두 전송하는 구조가 아닙니다. 보호자 알림은 WorkManager 주기 작업으로 **15분 이상 지연될 수 있습니다**. 이 설명이 앱 소스나 설치 파일의 공개·제공을 뜻하지는 않습니다.
...
```text
별도 호환 클라이언트 ── HTTPS ── 역방향 프록시/TLS ── Node.js 서버 ── SQLite·첨부 파일
초대 웹페이지 ───────── HTTPS ────────────────┘
                                             └─ 선택: 운영자가 별도 확보한 APK 제공
```
...
- `distribution/`: 선택적으로 운영자가 별도 확보한 APK를 두는 디렉터리. 저장소에는 APK가 없습니다.
...
별도 Android 클라이언트의 NAVER 지도 요청은 기기 IP와 조회 영역을 지도 제공자에게 전달합니다. 외부 TLS 프록시·터널을 선택하면 그 제공자도 통신 경로와 신뢰 경계에 포함됩니다. 초대 URL과 기기 토큰은 공개하지 말고, 프록시 접근 로그에도 초대 코드 등 민감한 정보가 남을 수 있음을 고려하세요. 자세한 내용은 [SECURITY.md](SECURITY.md)를 읽어 주세요.
...
| 별도 클라이언트 | 실제 휴대전화 사용 시에만 필요합니다. 호환 앱과 설치 경로는 따로 확보해야 하며 이 저장소가 제공하지 않습니다. |

**JDK, Android SDK, Gradle, NAVER 지도 키, APK 서명 키는 서버 실행·첫 초대 발급·API 테스트의 준비물이 아닙니다.** 서버는 지도 키 없이 실행됩니다. 서버/VPS 또는 전력, 도메인, 네트워크, 백업, 선택한 터널과 APK 전송량에 비용이 들 수 있습니다. 별도 클라이언트가 사용하는 외부 서비스 비용은 해당 제공자의 조건을 확인하세요.
...
이 절은 **서버만의 검증**입니다. 앱을 제공하거나 휴대전화 위치 공유·지도·알림 동작을 증명하지 않습니다. 로컬 서버는 첫 터미널에서 Ctrl+C로 정상 종료합니다. 로컬 데이터는 운영 데이터와 섞지 마세요.
...
**HTTP는 개발 테스트에만 사용하세요.** 서버의 HTTP 초대 허용은 클라이언트의 HTTP 지원을 보장하지 않습니다. 별도 안심연결 release 앱은 HTTPS만 허용합니다. 운영 환경의 TLS 오류를 인증서 검증 해제나 HTTP 전환으로 우회하지 마세요.

## 4. 첫 기기 초대와 가족 연결

첫 기기 초대는 **사용자가 0명인 서버에서만** 발급할 수 있는 로컬 운영자 작업입니다. APK 없이 발급할 수 있지만, 실제 참여에는 별도 호환 클라이언트가 필요합니다. 운영 서버와 **같은 데이터 저장소·환경 변수**를 사용하세요.
...
클라이언트를 별도로 확보한 경우의 연결 절차:

1. 첫 휴대전화에 호환 앱을 설치합니다. 운영자가 선택적으로 APK를 호스팅한다면 초대 페이지에서 내려받을 수 있습니다. 출처와 서버 운영자를 확인하고 설치를 직접 승인하세요. 이 저장소나 Google Play에서 현재 앱을 받을 수 있다는 뜻은 아닙니다.
...
- 별도 Android 클라이언트는 암호화한 기기 세션을 앱 종료·정상 업데이트 후에도 유지합니다. 다만 **업데이트·재부팅·설치 후 또는 강제 종료 후에는 앱을 직접 열고 공유 상태를 확인**하세요. 위치 서비스가 자동으로 재개된다고 가정하지 말고 필요하면 다시 동의해 공유를 켜세요.
...
- 앱 제거·데이터 지우기는 서버 프로필 삭제가 아닙니다. 별도 Android 클라이언트의 로컬 기기 식별 정보와 Android Keystore 키를 잃으면 이전 프로필로 돌아갈 수 없습니다. 자동 백업은 비활성화되어 있으며 환경설정 파일만 복사해도 복호화 키가 복원되지 않습니다. 기기 자격 증명과 서버 데이터가 일관되게 복원되지 않는다면 새 초대는 **새 프로필**을 만듭니다. 이름만 같게 입력해도 복구되지 않습니다.
- 모든 기기 자격 증명을 잃었는데 서버에 사용자가 남았다면 첫 기기 CLI로 우회할 수 없습니다. 데이터를 지우기 전에 운영자와 백업·기존 프로필 처리 방법을 확인하세요.

## 선택 사항: 별도 확보한 APK 호스팅

이 저장소는 APK를 제공하거나 빌드하지 않습니다. 운영자가 **배포 권한과 호환성을 확인한 설치 파일을 별도로 확보했을 때만** 기존 다운로드 경로를 사용할 수 있습니다. APK가 없어도 서버 시작·첫 초대 발급·API 테스트는 가능합니다.
...
파일이 없거나 경로가 미설정·읽기 불가인 경우 다운로드 요청은 **HTTP 503**을 반환하고 초대 페이지는 다운로드 버튼을 숨깁니다. 이미 설치한 앱으로 초대를 여는 안내는 유지됩니다. APK 미제공으로 인한 503은 서버 시작 실패를 뜻하지 않습니다. 앱 자동 업데이트 기능도 없으므로 별도 클라이언트 공급자의 업데이트 안내를 따르세요.
...
`/health` 성공은 서버 HTTP 응답만 확인합니다. 지도 인증, 기기 권한, 위치 공유·알림 전달이나 데이터 복구까지 증명하지 않습니다. **호환 클라이언트를 확보한 뒤에만** 운영자 본인의 두 기기로 초대·별도 공유 동의·위치 확인·공유 중지의 전체 흐름을 확인할 수 있습니다. 공개 이슈에 실사용 데이터나 초대 링크를 첨부하지 마세요.
...
직접 Node는 기존 서비스를 정상 종료한 뒤 `server/`에서 같은 환경으로 `node --env-file=../.env src/index.mjs`를 실행하거나 구성한 서비스 관리자로 재시작합니다. **기존 데이터 경로·Compose 프로젝트 이름·volume을 유지**하세요. 서버 업데이트는 별도 클라이언트의 설치·업데이트를 수행하지 않습니다.
...
| `/health`는 정상인데 휴대전화에서 쓸 앱이 없음 | 서버와 클라이언트는 별도입니다. 이 공개 저장소에는 앱 소스·APK가 없고 Google Play에도 아직 출시되지 않았습니다. 호환 클라이언트를 따로 확보해야 합니다. |
...
| 별도 앱의 지도 오류·업데이트 설치 실패 | 클라이언트 공급자에게 지도 설정 또는 패키지·서명·버전 호환성을 문의하세요. 서버에 지도 키를 추가할 문제는 아닙니다. 앱 삭제·데이터 초기화를 바로 권하지 마세요. 기기 신원을 잃고 서버의 이전 프로필이 남습니다. |
...
이 저장소의 공개 서버 소스는 [MIT License](LICENSE)를 따릅니다. **Android 앱 소스·바이너리는 이 공개 범위와 라이선스 허여 대상에 포함되지 않습니다.** 외부 구성 요소의 라이선스와 고지는 [NOTICE](NOTICE)를 참고하세요.

기여 절차는 [CONTRIBUTING.md](CONTRIBUTING.md), 보안 경계와 비공개 제보 안내는 [SECURITY.md](SECURITY.md)를 참고하세요. 서버 개선 제안과 재현 자료에는 실제 위치·토큰·초대 링크·개인 식별 정보를 넣지 마세요.
