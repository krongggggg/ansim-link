...
- **보호자는 연결된 피보호자**가 공유 중인 최근 위치와 최근 7일 범위의 위치 기록을 조회할 수 있습니다. 피보호자에게 보호자 위치·이동 기록·안심귀가·위치가 포함된 SOS를 제공하지 않으며, 보호자끼리 또는 피보호자끼리도 상대 위치를 조회할 수 없습니다.
- 피보호자 본인의 안심존 진입·이탈, 안심귀가 도착·예정 시간 경과, 장시간 움직임 미확인·위치 수신 중단에 관한 상태와 이벤트를 제공합니다. 가족의 안심존을 대신 관리하는 기능은 아닙니다. 위치가 오지 않는 것과 움직이지 않는 것은 다릅니다.
- 안심존과 안심귀가 목적지는 위도·경도 입력 없이 지도 중심을 움직여 선택하고, 지도에 표시되는 원을 보며 반경을 조정합니다. 이 흐름에는 올바르게 설정된 NAVER Maps Key ID와 네트워크 연결이 필요합니다.
- **가족 연결만으로 위치 공유가 시작되지 않습니다.** 피보호자의 위치 공유와 보호자 기기의 백그라운드 알림은 별도 설정입니다.
- FCM/푸시 기반의 실시간 전달 보장, 상용 BLE 반지 연동, 카카오톡 발송은 없습니다. 서버에는 기존 SOS·첨부 데이터용 API도 있으므로 해당 데이터까지 보호해야 합니다.

어딧 앱은 Android 전용이며 iOS 앱은 없습니다. NAVER 지도 표시와 위치·알림 권한은 앱에서 처리하고, 카메라 권한은 사용자가 **초대 QR 스캔을 선택했을 때만** 요청합니다. Android 화면에는 SOS 촬영·녹음·전송 기능이 없습니다. 피보호자의 공유 중 위치 서비스는 알림을 표시하는 포그라운드 서비스로 동작하며, 오프라인 위치를 디스크에 쌓아 나중에 모두 전송하는 구조가 아닙니다. 보호자 알림은 WorkManager 주기 작업으로 **15분 이상 지연될 수 있습니다**.
