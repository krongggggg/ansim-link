...
초대로 보호자·피보호자를 역할별로 연결하고, **피보호자가 직접 켠 위치 공유**를 바탕으로 안심존과 안심귀가를 관리합니다.
...
- **역할별 가족 그룹**: 초대할 때 보호자 또는 피보호자 역할을 지정합니다. 초대를 수락하면 두 사람의 기존 가족 그룹 전체가 하나로 합쳐집니다.
- **QR 초대**: 가족이 준비되면 역할이 지정된 1회용 초대를 발급합니다. 초대 링크에는 연결할 서버 정보가 포함됩니다.
- **안심존·안심귀가**: 각 구성원이 자신의 구역과 귀가 일정을 관리합니다. 동작에는 해당 기기의 위치 공유가 필요합니다.
...
- 가족 초대 QR을 앱에서 스캔하면 서버 주소나 16자리 코드를 다시 입력하지 않고 초대한 가족·서버·지정 역할 확인 화면으로 이동합니다. 초대 링크 또는 서버 주소와 코드를 직접 입력할 수도 있습니다. 이메일·비밀번호·일반 회원가입·로그인·로그아웃은 없습니다.
- **보호자**는 본인과 같은 가족 그룹의 보호자·피보호자가 각자 동의해 공유한 최근 위치와 최근 7일 위치 기록을 모두 조회할 수 있습니다. **피보호자**는 본인과 같은 그룹의 피보호자 공유 위치를 볼 수 있지만, 보호자의 위치·이동 기록·안심귀가·위치가 포함된 SOS는 볼 수 없습니다.
- 각 프로필은 안심존 진입·이탈, 안심귀가 도착·예정 시간 경과, 장시간 움직임 미확인·위치 수신 중단에 관한 상태와 이벤트를 제공합니다. 가족의 안심존을 대신 관리하는 기능은 아닙니다. 위치가 오지 않는 것과 움직이지 않는 것은 다릅니다.
- 안심존과 안심귀가 목적지는 위도·경도 입력 없이 지도 중심을 움직여 선택하고, 지도에 표시되는 원을 보며 반경을 조정합니다. 이 흐름에는 올바르게 설정된 NAVER Maps Key ID와 네트워크 연결이 필요합니다.
- **가족 그룹 연결만으로 위치 공유가 시작되지 않습니다.** 각 기기의 위치 공유와 보호자 기기의 백그라운드 알림은 별도 설정입니다. 역할별 초대는 한 사람이 한 번 사용하며, 수락하면 초대한 사람과 초대받은 사람의 기존 가족 그룹 전체가 합쳐집니다.
- FCM/푸시 기반의 실시간 전달 보장, 상용 BLE 반지 연동, 카카오톡 발송은 없습니다. 서버에는 기존 SOS·첨부 데이터용 API도 있으므로 해당 데이터까지 보호해야 합니다.

어딧 앱은 Android 전용이며 iOS 앱은 없습니다. NAVER 지도 표시와 위치·알림 권한은 앱에서 처리하고, 카메라 권한은 사용자가 **초대 QR 스캔을 선택했을 때만** 요청합니다. Android 화면에는 SOS 촬영·녹음·전송 기능이 없습니다. 위치 공유를 켠 기기의 위치 서비스는 알림을 표시하는 포그라운드 서비스로 동작하며, 오프라인 위치를 디스크에 쌓아 나중에 모두 전송하는 구조가 아닙니다. 보호자 알림은 WorkManager 주기 작업으로 **15분 이상 지연될 수 있습니다**.
...
adb shell am start -n org.ansim.link/.MainActivity
```
...
여러 기기가 연결되어 있으면 모든 `adb` 명령에 `-s <기기 일련번호>`를 지정하세요. 덮어쓰기 설치 중에는 위치 수집이 잠시 중단됩니다. 설치 후에는 기기 잠금을 해제하고 앱을 열어 공유 상태를 확인하세요.
...
2. 설치 뒤 어딧 첫 화면의 **‘초대 QR 스캔’으로 가족 폰에 받은 같은 QR을 다시 비추거나**, 같은 초대 웹페이지로 돌아와 ‘앱에서 초대 열기’를 선택합니다. 서버 주소나 16자리 코드를 다시 입력할 필요가 없습니다. 설치만으로 브라우저의 초대 상태가 앱에 자동 이전되지는 않습니다.
3. 앱에 표시된 초대한 가족·서버·지정 역할을 확인합니다. 처음 참여하는 기기는 표시 이름을 정한 뒤 해당 역할로 직접 연결합니다. 첫 기기 설정 초대는 보호자 역할이며, 이름은 인증 비밀번호가 아닙니다.
4. 참여한 기기에서 **초대받을 사람의 보호자·피보호자 역할을 선택해** 새 가족 초대 QR/링크를 만들고 상대방에게 비공개로 전달합니다. 이미 같은 서버에 참여한 기기도 현재 프로필과 같은 역할용 초대만 사용해 연결을 추가할 수 있습니다.
5. 초대를 수락하면 초대한 사람과 받은 사람의 **기존 가족 그룹 전체가 하나로 합쳐집니다**. 같은 그룹 구성원은 다시 연결할 수 없으며, 그룹은 본인을 제외해 최대 100명입니다.
6. 각 기기는 **별도로 위치 공유에 동의하고 켭니다**. 보호자 위치는 그룹의 보호자만 볼 수 있고, 피보호자 위치는 그룹의 보호자와 피보호자가 볼 수 있습니다.
...
버전 1.3 이하 서버의 기존 프로필은 v3 데이터베이스 이전 뒤 역할이 미선택 상태가 됩니다. 이 상태에서는 다른 사람 위치·이동 기록·위치가 포함된 안전 정보가 숨겨지고 새 위치 전송도 거부됩니다. Android 1.4 이상에서 각 기기가 보호자 또는 피보호자를 한 번 선택해야 하며 앱에서 다시 바꿀 수 없습니다. 역할을 선택해도 기존 위치 기록과 진행 중인 안심귀가는 유지됩니다. 역할 선택 뒤에는 기존 공유 동의를 확인하고 이 기기의 위치 수집을 직접 다시 시작해야 할 수 있습니다. 운영자는 먼저 일관된 저장소 백업을 만든 뒤 서버와 Android 앱을 함께 업데이트해야 합니다.
...
- Android 앱은 암호화한 기기 세션을 앱 종료·정상 덮어쓰기 업데이트 후에도 유지합니다. 역할 이전 뒤에는 공유 상태와 권한을 확인하고 위치 수집을 직접 다시 시작해야 할 수 있습니다. 꺼 둔 공유·해제 확인 대기 상태를 자동으로 켜지 않으며, 서버 상태를 확인할 수 없으면 재개하지 않습니다.
- 각 프로필의 위치 기록은 서버에서 7일 보관 범위로 관리됩니다. 공유를 끄면 클라이언트는 새 전송을 중단하고 서버에 저장된 해당 프로필의 위치 기록 삭제를 요청합니다. 서버 반영 시 진행 중·예정 시간 경과 상태의 안심귀가도 취소됩니다. **오프라인이면 서버 반영은 보류**되므로 클라이언트의 완료 상태를 확인해야 합니다. 공유 중지는 프로필·가족 그룹·모든 종류의 데이터 삭제와 같지 않습니다.
- 가족 그룹은 직접 연결들의 그래프로 구성됩니다. 직접 연결 하나를 해제하면 그 연결을 통해 이어진 하위 그룹도 분리될 수 있지만 상대방의 프로필이나 서버의 위치 기록은 삭제하지 않습니다. 간접 구성원은 연결 경로를 만든 기기에서 직접 연결을 해제해야 합니다.
...
`adb install -r`은 지정한 기기만 업데이트하며 서버의 다운로드 APK는 바꾸지 않습니다. QR 초대 페이지에서도 새 버전을 제공하려면 아래 파일 게시를 별도로 완료하세요. 게시 후에는 공개 다운로드 주소에서 실제 APK를 내려받아 준비한 파일과 SHA-256·버전·서명이 일치하는지 확인합니다.
...
**서버 버전 변경 전에 일관된 백업을 확보하세요.** 서버 시작 시 현재 스키마 v3로 이전하며, v1/v2의 사용자 ID·가족 연결·데이터와 아직 유효한 세션을 유지합니다. 기존 연결 그래프의 모든 연결 경로를 하나의 가족 그룹으로 계산합니다. 이전된 유효 세션은 지속 기기 자격 증명이 되고 이미 만료된 세션은 되살리지 않습니다. 지원 범위를 넘는 DB 버전이나 버전 정보가 없는 기존 DB는 자동 변경하지 않습니다. DB 다운그레이드는 보장하지 않습니다.
...
| 설치 후 초대가 안 보이거나 코드를 다시 요구함 | 어딧 첫 화면의 ‘초대 QR 스캔’으로 가족 폰에 받은 같은 QR을 다시 비추거나 원래 초대 페이지로 돌아와 앱 열기를 누릅니다. QR에는 서버 주소와 코드가 함께 들어 있으므로 직접 다시 입력할 필요가 없습니다. 설치만으로 브라우저의 초대 상태가 앱에 자동 이전되지는 않습니다. |
