...
`android/store/`의 스토어 소재·스크린샷·지도 캡처는 지도 콘텐츠 재배포 조건을 별도로 검토해야 하므로 공개 범위에서 제외합니다. 로컬 SDK 설정·키·캐시·빌드 결과·운영 데이터도 제외합니다. 기존 설치와 서버 호환성을 위해 패키지 ID `org.ansim.link`, 초대 프로토콜 `ansimlink://`, DB·저장소 이름, APK 경로 `/downloads/ansim-link.apk` 및 저장소 URL의 `ansim-link`는 유지합니다. 제품 이름은 어딧(Eodit)입니다.
